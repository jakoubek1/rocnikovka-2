...
export default function Main() {
  ...
  const load = async () => {
    ...
    console.log("Hledaný parametr:", nameParam);

    const data = await getAllReservation(nameParam);
        ...
        <main className="flex-grow px-6 md:px-12 lg:px-24 py-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-8 gap-y-12 justify-center items-start">
            {reservation && reservation.length > 0 ? (
              ...
              <p className="text-center text-gray-400 col-span-full">Žádné rezervace k zobrazení.</p>
            )}
          </div>
        </main>
      </div>
